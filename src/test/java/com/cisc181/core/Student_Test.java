...
public class Student_Test {
	...
	public static void setup() {
		...
		courses.add(new Course("History", 8));
		courses.add(new Course("English", 10));

		ArrayList<Semester> semesters = new ArrayList<Semester>();
		Calendar startDate = Calendar.getInstance();
		Calendar endDate = Calendar.getInstance();
		startDate.set(2016, 8, 1);
		endDate.set(2016, 12, 1);
		semesters.add(new Semester(startDate.getTime(), endDate.getTime()));
		startDate.set(2016, 1, 1);
		endDate.set(2016, 5, 1);
		semesters.add(new Semester(startDate.getTime(), endDate.getTime()));
	...
	public void testGPA() {
		...
		for (int i = 0; i < students.size(); i++) {
			Enrollment enrollment = new Enrollment(students.get(i)
					.getStudentID(), sections.get(i % 6).getSectionID());
	...
	public void testMajor() {
		...
		assertEquals(eMajor.PHYSICS, students.get(0).getMajor());
	}
}